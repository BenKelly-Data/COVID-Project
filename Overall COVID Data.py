...
#Percent of population which contracted COVID
#Does not account for multiple bouts of COVID in one person
covidPercent =  psql.sqldf("SELECT location, date,total_cases,population, (total_cases/population)*100 as COVIDPercent FROM deaths WHERE continent IS NOT NULL ORDER BY 1,2")
covidPercent
...
propInfectionPercent =  psql.sqldf("SELECT location,population,max(total_cases) AS highestCount, max((total_cases/population))*100 AS proportionalPercent FROM deaths WHERE continent IS NOT NULL GROUP BY location, population ORDER BY proportionalPercent DESC")
propInfectionPercent
...
#Global Numbers
globalDeaths =  psql.sqldf("SELECT SUM(new_cases) AS total_cases ,SUM(new_deaths) AS total_deaths, (SUM(new_deaths)/SUM(new_cases))*100 as DeathPercentage FROM deaths WHERE continent IS NULL ORDER BY 1,2")
globalDeaths
